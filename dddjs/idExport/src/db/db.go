...
func Export_ids()error{
	...
	err = playerC.Find(bson.M{}).Select(bson.M{"pid":1}).All(&results)
	if err != nil {
		sglog.Error("find all pid error", err)
		return err
	}
	sglog.Info("total pid:",len(results))
	printStrList:=[]string{}
	for _,v:=range results{
		mapstr,ok:=v.(map[string]string);
		if ok{
			sglog.Info("transform ok:",mapstr)
		}
	}

	sglog.Info("array:",printStrList)
